...
get_SubstrateCEPs(Dimensions,Plasticity)->
	case Plasticity of
		...
		abcn ->
			[#actuator{name=abcn,type=substrate,vl=4}]; %[{abcn,4}];
		none ->
...
discrete_tmaze(sensors)->
	[#sensor{name=dtm_GetInput,type=standard,scape={private,dtm_sim},vl=VL,parameters=[Paramter]} || {VL,Paramter} <- [{1,reward},{3,range_sense}]];
discrete_tmaze(actuators)->
	[
		%#actuator{name=dtm_SendOutput,type=standard,scape={private,dtm_sim},vl=1,parameters=[]}
		#actuator{name=dtm_SubstrateSendOutput,type=standard,scape={private,dtm_sim},vl=3,parameters=[]}
	].
